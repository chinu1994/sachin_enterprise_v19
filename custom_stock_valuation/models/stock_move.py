...
class StockMove(models.Model):
    ...
    def backfill_custom_stock_valuation(self):
        """
        Public XML-RPC wrapper for backfilling stock valuation
        on already-done stock moves.

        This method intentionally reuses the existing valuation engine
        instead of creating journal entries directly.
        """
        ...
        for move in self:
            ...
            # Already valued -> do nothing
            if move.valuation_move_id:
                continue

            # Custom stock valuation must be enabled
            if not move.company_id.custom_stock_valuation_enabled:
                continue

            # Internal transfers must be skipped
            if (
                    move.picking_id
            ...
            # Use the existing valuation engine
            move._generate_valuation_entries()
